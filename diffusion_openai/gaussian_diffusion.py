...
class GaussianDiffusion:
    ...
    def ddim_sample_loop_progressive(
        ...
        #saver = cond_kwargs["saver"] @riccardo testing ddim error
        resampling_steps = 1
        cond_frames = cond_kwargs["cond_frames"]
        if cond_frames:
            cond_img = cond_kwargs["cond_img"]
    ...
    def training_losses(
        ...
        for b in range(x_t.shape[0]):
            ...
            if unconditional:
                ...
                #@riccardo settaggio intervalli fissi di masking a training, commento il generatore di intervalli random
                #r = random.randint(1, max_num_mask_frames)
                #mask = random.sample(range(*mask_range), r)
                mask=[]
                for e in range(x_t.shape[2]):
                    if e ==0:
                        continue
                    elif e%10 != 0 and e%10 != 1:        
                        mask.append(e)
                # fine modifica     
